//! Биграммная модель для выбора следующего токена
//!
//! Модель связывает статистику переходов, источник
//! псевдослучайных значений и выборки:
//!
//! ```text
//! current TokenId
//!     ↓
...
/// Биграммная модель, выбирающая следующий токен
...
impl BigramModel {
    /// Создаёт биграммную модель из готовой статистики переходов
    ...
    /// получает случайное значение из [`RandomSource`] и выполняет выборку
    ///
    /// # Errors
    ///
    /// Возвращает [`BigramModelError`], если:
    /// - не удалось получить распределение вероятностей
    /// - выборка завершилась ошибкой
...
/// Ошибки, возникающие при работе биграммной модели
///
...
pub enum BigramModelError {
    /// Ошибка при работе со статистикой биграммных переходов
