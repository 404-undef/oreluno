...
//! значений, который позволяет в дальнейшем подменять реализацию генератора
...
/// 64-битную функцию перемешивания Stafford, вариант 13
///
...
impl Rng {
    ...
    ///
    /// Значение `0` является допустимым начальным состоянием и не требует специальной
    /// обработки
    ...
    /// перемешивается функцией Stafford, вариант 13
    ///
...
///
/// Типаж отделяет потребителя случайных чисел от конкретной реализации
/// генератора. Благодаря этому алгоритмы могут использовать [`Rng`] либо
/// другую пользовательскую или промышленную реализацию, не меняя свою
/// основную логику
///
